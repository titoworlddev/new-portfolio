...
export async function generateCV(cvData: CVData) {
  try {
    ...
    pdf.setFont('helvetica', 'normal');
    pdf.text(
      `• Correo electrónico: ${cvData.personalInfo.email}`,
      20,
      yPosition
    );
    yPosition += 5;
    ...
    if (cvData.skills.web && cvData.skills.web.length > 0) {
      pdf.text(
        `• Desarrollo Web: ${cvData.skills.web.join(', ')}`,
        20,
        yPosition
      );
      yPosition += 5;
    ...
    if (cvData.skills.utilities && cvData.skills.utilities.length > 0) {
      pdf.text(
        `• Herramientas: ${cvData.skills.utilities.join(', ')}`,
        20,
        yPosition
      );
      yPosition += 5;
    ...
    if (cvData.skills.languages && cvData.skills.languages.length > 0) {
      ...
      pdf.text(
        `• Idiomas: Español (Nativo), Inglés (Básico), Valenciano (Alto), Catalán (Alto)`,
        20,
        yPosition
      );
    }
